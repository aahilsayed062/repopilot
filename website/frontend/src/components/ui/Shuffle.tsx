...
const Shuffle = ({
        ...
        const initGSAP = async () => {
            try {
                ...
                try {
                    ...
                    ScrollTriggerPlugin = scrollModule.ScrollTrigger;
                } catch {
                ...
                if (SplitTextPlugin) gsap.registerPlugin(SplitTextPlugin);
                ...
                const teardown = () => {
                    ...
                    if (wrappersRef.current.length) {
                        wrappersRef.current.forEach((wrap: HTMLSpanElement) => {
                ...
                const build = () => {
                    ...
                    try {
                        if (!SplitTextPlugin) {
                            // Fallback: no split text, just show the text
                            setReady(true);
                            onShuffleComplete?.();
                            return;
                        }
                        ...
                        console.error("Shuffle animation error:", err);
                    }
                };
                ...
                const play = () => {
                    ...
                    if (animationMode === 'evenodd') {
                        const odd = strips.filter((_: unknown, i: number) => i % 2 === 1);
                        const even = strips.filter((_: unknown, i: number) => i % 2 === 0);
                        const oddTotal = duration + Math.max(0, odd.length - 1) * stagger;
                        const evenStart = odd.length ? oddTotal * 0.7 : 0;
                        if (odd.length) addTween(odd, 0);
                        if (even.length) addTween(even, evenStart);
                    } else {
                        strips.forEach((strip: HTMLElement) => {
                ...
                if (ScrollTriggerPlugin) {
                    const st = ScrollTriggerPlugin.create({ trigger: el, start: scrollTriggerStart, once: triggerOnce, onEnter: create });
                    return () => {
                        st.kill();
                        removeHover();
                        teardown();
                    ...
                    // No ScrollTrigger — just play immediately
                    create();
                    return () => {
                        removeHover();
                        teardown();
                ...
                console.error("Shuffle init error:", err);
                setReady(true);
            }
        };
    ...
    const baseTw = 'inline-block whitespace-nowrap will-change-transform leading-none';
