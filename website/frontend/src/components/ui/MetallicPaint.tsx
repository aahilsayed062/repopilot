...
export default function MetallicPaint({ imageData, params = defaultParams, className = '' }: MetallicPaintProps) {
    ...
    function updateUniforms() {
        ...
        gl.uniform1f(uniforms.u_edge, mergedParams.edge);
        gl.uniform1f(uniforms.u_patternBlur, mergedParams.patternBlur);
        gl.uniform1f(uniforms.u_time, 0);
        gl.uniform1f(uniforms.u_patternScale, mergedParams.patternScale);
        gl.uniform1f(uniforms.u_refraction, mergedParams.refraction);
        gl.uniform1f(uniforms.u_liquid, mergedParams.liquid);
    }
    ...
    useEffect(() => {
        function initShader() {
            ...
            const vertexShader = createShader(glCtx, vertexShaderSource, glCtx.VERTEX_SHADER);
            const fragmentShader = createShader(glCtx, liquidFragSource, glCtx.FRAGMENT_SHADER);
            ...
            if (!glCtx.getProgramParameter(program, glCtx.LINK_STATUS)) {
                console.error('Unable to initialize the shader program: ' + glCtx.getProgramInfoLog(program));
                return;
            }
            ...
            const uniformMap = getUniforms(program, glCtx);
